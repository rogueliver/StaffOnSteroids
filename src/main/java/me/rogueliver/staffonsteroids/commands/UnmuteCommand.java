...
public class UnmuteCommand implements CommandExecutor {
    ...
    public UnmuteCommand(StaffOnSteroids plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            sender.sendMessage(MessageUtil.error("errors.usage-unmute"));
            return true;
        }
        ...
        if (!plugin.getDatabaseManager().getMutedPlayers().contains(target.getUniqueId())) {
            sender.sendMessage(MessageUtil.error("errors.player-not-muted"));
            return true;
        }

        plugin.getDatabaseManager().getMutedPlayers().remove(target.getUniqueId());
        plugin.getDatabaseManager().removeTempMute(target.getUniqueId());
