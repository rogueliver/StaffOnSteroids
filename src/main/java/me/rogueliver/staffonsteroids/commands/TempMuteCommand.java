...
public class TempMuteCommand implements CommandExecutor {
    ...
    public TempMuteCommand(StaffOnSteroids plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(MessageUtil.error("errors.usage-tempmute"));
            return true;
        }
        ...
        plugin.getDatabaseManager().saveTempMute(target.getUniqueId(), reason, sender.getName(), System.currentTimeMillis() + durationMillis);

        target.sendMessage(MessageUtil.info("info.muted", "reason", reason));
        sender.sendMessage(MessageUtil.success("success.temp-muted", "player", targetName, "duration", TimeUtil.formatDuration(duration), "reason", reason));
